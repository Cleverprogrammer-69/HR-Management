...
import {
  getOneDepartment,
  updateDepartment,
} from '@/lib/store/features/department/departmentSlice';
...
export default function Page() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      toast({
        ...
        description: 'Department is updated successfuly.',
      });
    } catch (err) {
      toast({
        variant: 'destructive',
  ...
  useEffect(() => {
    async function fetchDepartment() {
      try {
        const fetchedDepartment = await dispatch(
          getOneDepartment(params.id)
        ).unwrap();
        if (
          fetchedDepartment &&
          fetchedDepartment.data &&
          fetchedDepartment.data[0]
        ) {
          const department = fetchedDepartment.data[0];
